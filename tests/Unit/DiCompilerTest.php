...
$config = [
    "sitename" => "My Own Website",
    "comically_bad_default_password" => 'p@$$w0rd',
];
...
beforeEach(function () use ($config) {
    ...
    // Create the class
    eval(
        "namespace Tests\\Examples; class {$uniqueClass} extends ExampleContainer {}"
    );
...
test("it errors on faulty service definitions during compilation", function (
    $containerClass,
) {
    ...
    eval(
        "namespace Tests\\Examples; class " .
            $uniqueClass .
            " extends " .
            basename(str_replace("\\", "/", $containerClass)) .
            " {}"
    );
    $this->compiler = new DiCompiler($fullClassName);
    expect(fn() => $this->compiler->compile())->toThrow(
...
test("instance with config trait supports config values", function () {
    ...
    expect($user->password)->toBe(
        $instance->config("comically_bad_default_password"),
    );
});
